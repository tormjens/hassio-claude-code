 ...
 */
import { highlightToHtml, languageForFile } from './highlight';

export type DiffLine = { type: 'add' | 'del' | 'ctx'; text: string; html: string };
...
export function lineDiff(oldStr: string, newStr: string): DiffLine[] {
  ...
  if (a.length === 0) return b.map((text) => ({ type: 'add' as const, text, html: '' }));
  if (b.length === 0) return a.map((text) => ({ type: 'del' as const, text, html: '' }));
  if (a.length * b.length > 250_000) {
    return [
      ...a.map((text) => ({ type: 'del' as const, text, html: '' })),
      ...b.map((text) => ({ type: 'add' as const, text, html: '' })),
    ];
  }
  const m = a.length;
  const n = b.length;
  ...
  while (i < m && j < n) {
    if (a[i] === b[j]) {
      out.push({ type: 'ctx', text: a[i], html: '' });
      i++;
      j++;
    } else if (dp[i + 1][j] >= dp[i][j + 1]) {
      out.push({ type: 'del', text: a[i], html: '' });
      i++;
    } else {
      out.push({ type: 'add', text: b[j], html: '' });
      j++;
    }
  }
  while (i < m) out.push({ type: 'del', text: a[i++], html: '' });
  while (j < n) out.push({ type: 'add', text: b[j++], html: '' });
  return out;
}
...
export function fileEditView(name: string, input: unknown): FileEditView | null {
  ...
  for (const b of blocks) {
    for (const l of b.lines) {
      l.html = highlightToHtml(l.text, lang);
      if (l.type === 'add') adds++;
