...
export function buildSystemPromptAppend(config: AppConfig, extra: { haVersion?: string; gitEnabled: boolean }): string {
  return [
    ...
    'The `ha` MCP server gives you live access to the running instance. These tools are always available; call them by their exact names, which are prefixed with `mcp__ha__`:',
    '- mcp__ha__ha_get_areas_devices and mcp__ha__ha_get_states: resolve names like "the garage" to entity ids and inspect state. Prefer these over guessing entity ids.',
    '- mcp__ha__ha_get_services: discover services and their fields before calling them.',
